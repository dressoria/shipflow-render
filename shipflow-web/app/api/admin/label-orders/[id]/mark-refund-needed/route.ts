...
import { adminMarkRefundNeeded } from "@/lib/server/pendingLabelOrders";
...
export async function POST(
  ...
  try {
    ...
    const order = await adminMarkRefundNeeded(id, safeReason);

    return apiSuccess({ order });
  } catch (error) {
    return apiErrorFromUnknown(error, "We could not mark this order as refund_needed.");
  }
}
