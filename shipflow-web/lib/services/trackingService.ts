...
export async function getTrackingEvents(trackingNumber: string): Promise<TrackingEvent[]> {
  ...
  return [
    ...
    },
    {
      ...
      shipmentId: shipment.id,
      trackingNumber: shipment.trackingNumber,
      title: shipment.status === "Pendiente" ? "Waiting for carrier handoff" : "Collected by carrier",
      status: shipment.status,
      date: shipment.date,
    },
    {
      ...
      shipmentId: shipment.id,
      trackingNumber: shipment.trackingNumber,
      title: shipment.status === "Entregado" ? "Delivered to recipient" : "In transit to destination",
      status: shipment.status,
      date: shipment.date,
    },
  ];
