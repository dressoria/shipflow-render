      ...
      <section className="rounded-3xl border border-blue-100 bg-white p-5 shadow-sm shadow-slate-950/5">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-600">Managed Amazon FBA prep request · created {formatDate(order.createdAt)}</p>
          </div>
          <div className="rounded-2xl bg-orange-50 px-4 py-3 text-right">
            <p className="text-xs font-black uppercase tracking-widest text-orange-700">Estimated from</p>
            <p className="text-xl font-black text-slate-950">{formatCurrency(order.estimatedTotal ?? 0)}</p>
          </div>
        </div>
        <div className="mt-5 grid gap-3 md:grid-cols-4">
          ...
          <Summary label="Receiving ref" value={order.receivingReference || "Pending"} />
        </div>
      </section>
